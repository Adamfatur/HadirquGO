    ...
    <div class="container mt-5">
        <div class="text-center mb-5">
            <h2 class="fw-bold text-white" style="font-size: 1.5rem;">
                🏆 Top 50 Rankings by Duration 🏆
            </h2>
            <p class="text-light" style="font-size: 1rem; margin-top: -5px;">
                Check your performance and rankings based on <strong>total duration</strong>!
            </p>
        </div>
        ...
        <div class="d-flex justify-content-center mb-4">
            <form method="GET" action="{{ route('lecturer.viewboard.top-duration') }}" class="d-inline-block">
        ...
        <div class="card shadow-sm p-4 mb-5 animate__animated animate__fadeIn" style="border-radius: 20px; background-color: #f9f9fb;">
            <div class="card-header bg-transparent border-0 d-flex justify-content-between align-items-center">
                <h2 class="mb-0 text-secondary font-weight-bold" style="color: #1e3a8a;">
                    Top 50 Rankings by Duration - {{ ucfirst($period) }}
                </h2>
                <small class="text-muted">{{ __('Updated hourly') }}</small>
            </div>
            <div class="card-body">
                @include('partials.leaderboard_search', ['searchCategory' => 'top_duration_' . $period])
                @if($rankings->isEmpty())
                    <div class="text-center">
                        <i class="fas fa-trophy text-primary" style="font-size: 4rem;"></i>
                        <h3 class="text-dark fw-bold mt-3">{{ __('No Data Available') }}</h3>
                        <p class="text-muted mt-2">Try checking back later or selecting a different time period.</p>
                    </div>
                @else
                        ...
                        <table class="table table-hover align-middle mb-0">
                            ...
                            @foreach($rankings as $data)
                                <tr class="border-bottom animate__animated animate__fadeIn">
                                    ...
                                    <td class="fw-bold">
                                        @if($data->user)
                                            <a href="{{ route('lecturer.evaluation.show', ['member_id' => $data->user->member_id]) }}" class="text-dark text-decoration-none d-flex align-items-center">
...
@push('styles')
<style>
    .btn-hover-animate { transition: transform 0.2s; }
    .btn-hover-animate:hover { transform: translateY(-2px); box-shadow: 0 4px 8px rgba(0,0,0,0.2); }
    .btn-primary.active { background-color: #152c5b; border-color: #152c5b; }
    .table-hover tbody tr:hover { background-color: #e6f0ff; }
</style>
@endpush
