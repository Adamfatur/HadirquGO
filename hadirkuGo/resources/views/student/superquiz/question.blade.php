@extends('layout.student') {{-- Asumsikan menggunakan layout student --}}
    ...
    <div class="container py-5">
        @if(isset($resetLocalStorage) && $resetLocalStorage)
            <script>
                ...
                localStorage.removeItem('quizStartTime_question_{{ $questionNumber }}');
            </script>
        @endif
        ...
        <form action="{{ route('student.superquiz.submitAnswer', [$superQuiz->unique_id, $questionNumber]) }}" method="POST" id="quizForm"> {{-- Route action updated to student --}}
            @csrf
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function sendTimeoutRequest() {
                fetch("{{ route('student.superquiz.timeoutAttempt', $superQuiz->unique_id) }}", {  {{-- Route timeout diperbarui ke student --}}
                    method: 'POST',
                    headers: {
                    ...
                    .then(response => {
                        if (response.ok) {
                            ...
                            window.location.href = "{{ route('student.superquiz.viewResult', $superQuiz->unique_id) }}"; {{-- Route viewResult diperbarui ke student --}}
                        } else {
