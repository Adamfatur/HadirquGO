...
@section('content')
    <div class="container" style="padding: 20px; max-width: 400px; margin: auto; text-align: center; font-family: Arial, sans-serif;">
        <div class="card shadow" style="border-radius: 15px; overflow: hidden; background: #ffffff; padding: 20px; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.2);">
            <h2 style="color: #4A4A4A; font-weight: bold; margin-bottom: 20px;">{{ __('Check-In') }} QR Code</h2>

            <div style="background-color: #f8f9fa; padding: 20px; border-radius: 10px;">
                <img src="data:image/png;base64,{{ $qrCode }}" alt="{{ __('Check-In') }} QR Code" style="width: 200px; height: 200px; border-radius: 10px; box-shadow: 0px 8px 15px rgba(0, 0, 0, 0.1);">
            </div>
            ...
            <div style="margin-top: 15px; font-size: 13px; color: #1f2937; background-color: #e9ecef; padding: 10px; border-radius: 8px;">
                ...
                <p style="margin: 0;">This QR code is valid for <strong>{{ __('Check-In') }}</strong> only.</p>
...
<script>
    function checkActiveCheckinToken() {
        fetch("{{ route('lecturer.qrcode.checkActiveCheckinToken') }}", {
            method: "GET",
            headers: {
            ...
            .catch(error => {
                console.error("Error checking active checkin token:", error);
            });
    }

    // Cek status setiap 1.5 detik (sesuai kode Anda)
    setInterval(checkActiveCheckinToken, 1500);
</script>