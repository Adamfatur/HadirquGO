...
class SuperQuizController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('student.superquiz.index', compact('superQuizzes'));
    }
    ...
    public function viewResult(SuperQuiz $superQuiz)
    {
        ...
        if (!$attempt) {
            return redirect()->route('student.superquiz.index')->with('error', 'You have not taken this quiz today.');
        }
        ...
        return view('student.superquiz.viewResult', compact('superQuiz', 'score', 'correctAnswers', 'results'));
    }
    ...
    public function show(SuperQuiz $superQuiz)
    {
        ...
        if ($todayAttempt) {
            return redirect()->route('student.superquiz.index')
        ...
        return redirect()->route('student.superquiz.question', [
    ...
    public function showQuestion(SuperQuiz $superQuiz, $questionNumber)
    {
        ...
        // Cek apakah ID soal dan attempt ada di session
        if (!$questionIds || !isset($questionIds[$questionIndex])) {
            // Kuis selesai atau terjadi kesalahan
            return redirect()->route('student.superquiz.index')
                ->with('success', 'Super Quiz is over!');
        }

        // Ambil soal berdasarkan ID yang ada di session
        ...
        // Cek jika hari sudah berganti, reset session timer dan data kuis
        $lastAttemptDate = session('last_attempt_date', Carbon::today()->toDateString());
        $todayDate = Carbon::today()->toDateString();

        // Jika tanggal berbeda, reset session untuk memulai ulang kuis
        if ($lastAttemptDate !== $todayDate) {
            ...
            // Menyimpan tanggal hari ini ke session
            session(['last_attempt_date' => $todayDate]);
        ...
        // Set waktu mulai soal saat ini di sesi
        session(['question_start_time' => time()]);

        // Kirim data ke view untuk ditampilkan
        return view('student.superquiz.question', compact('superQuiz', 'question', 'questionNumber', 'timeLeft', 'potentialScore'));
    }
    ...
    public function showConfirmation(SuperQuiz $superQuiz, $questionNumber)
    {
        ...
        return view('student.superquiz.confirmation', compact('superQuiz', 'questionNumber', 'quizScore', 'potentialScore')); // Pass potentialScore
    }
    ...
    public function submitAnswer(Request $request, SuperQuiz $superQuiz, $questionNumber)
    {
        ...
        if (!$questionStartTime) {
            ...
            return redirect()->route('student.superquiz.index')->with('error', 'Invalid quiz session. (No start time)');
        }
        ...
        if ($elapsedTimeForQuestion >= 10) {
            ...
            return redirect()->route('student.superquiz.failed', [
                'superQuiz' => $superQuiz->unique_id,
                'questionNumber' => $questionNumber,
            ])->with('error', 'Time is up for this question.');
        }
        ...
        if ($isCorrectAnswer) {
            ...
            if ($nextQuestionIndex >= count($questionIds)) {
                ...
                return redirect()->route('student.superquiz.confirmation', [
                    'superQuiz' => $superQuiz->unique_id,
                    'questionNumber' => $questionNumber,
    ...
    {
        return view('student.superquiz.failed', compact('superQuiz', 'questionNumber'));
    }
    ...
    public function timeoutAttempt(SuperQuiz $superQuiz)
    {
        ...
        $attempt = SuperQuizAttempt::where('unique_id', $attemptUniqueId)->first();
        if (!$attempt) {
            return redirect()->route('student.superquiz.index')->with('error', 'Attempt not found.');
        }
        ...
        if ($elapsedTime >= 10) {
            ...
            return redirect()->route('student.superquiz.viewResult', [$superQuiz->unique_id])->with('error', 'Time has expired for this question.');
        }
