...
@section('content')
    <div class="container py-4">
        ...
        <div class="d-flex justify-content-center align-items-center flex-column">
            <div class="card p-4 shadow-sm w-100" style="max-width: 500px; border-radius: 20px;">
                <div class="text-center">
                    ...
                    <div class="position-relative d-inline-block">
                        <img src="{{ $user->avatar ?? asset('images/default.jpg') }}"
                             alt="User Avatar"
                             class="rounded-circle {{ $rankStyle && $profileData['rank'] <= 50 ? $rankStyle['class'] : '' }}"
                    ...
                    <p class="mt-2 text-muted font-italic">
                        "{{ optional($user->biodata)->about ?? 'No bio available.' }}"
                    </p>
                </div>
                ...
                <div class="row g-2 mb-3 text-center">
                    <div class="col-3">
                        <div class="p-2 rounded-3" style="background: linear-gradient(135deg, #00f2fe, #4facfe); color: white;">
                            ...
                            <div style="font-size: 0.65rem;">Level</div>
                        </div>
                    </div>
                    <div class="col-3">
                        <div class="p-2 rounded-3" style="background: linear-gradient(135deg, #f59e0b, #ef4444); color: white;">
                            ...
                            <div style="font-size: 0.65rem;">Rank</div>
                        </div>
                    </div>
                    <div class="col-3">
                        <div class="p-2 rounded-3" style="background: linear-gradient(135deg, #10b981, #34d399); color: white;">
                            ...
                            <div style="font-size: 0.65rem;">Badges</div>
                        </div>
                    </div>
                    <div class="col-3">
                        <div class="p-2 rounded-3" style="background: linear-gradient(135deg, #8b5cf6, #a78bfa); color: white;">
                            ...
                            <div style="font-size: 0.65rem;">Points</div>
                        </div>
                    </div>
                </div>
                <div class="text-center mb-3">
                    <span class="badge rounded-pill px-3 py-1" style="background: #eff6ff; color: #1d4ed8; border: 1px solid #93c5fd; font-size: 0.8rem;">
                        {{ $profileData['level_name'] }} &middot; {{ $profileData['total_sessions'] }} Sessions
                    </span>
                </div>
                ...
                <div class="px-3">
                    <p><strong>Phone Number:</strong> {{ optional($user->biodata)->phone_number ?? '-' }}</p>
                    <p><strong>ID Number:</strong> {{ optional($user->biodata)->id_number ?? $user->member_id ?? '-' }}</p>
                    <p><strong>Other ID Number:</strong> {{ optional($user->biodata)->other_id_number ?? '-' }}</p>
                    <p><strong>Nickname:</strong> {{ optional($user->biodata)->nickname ?? '-' }}</p>
                ...
                <div class="text-center">
                    ...
                    @if(!$user->name_changed)
                        <button class="btn btn-outline-info w-100 mb-2" data-bs-toggle="modal" data-bs-target="#changeNameModal">
                            <i class="fas fa-signature me-2"></i>Change Name (1x only)
                        </button>
                    @endif

                    @if(!$user->biodata || !$user->biodata->birth_date)
                        <button class="btn btn-secondary w-100 mb-2" data-bs-toggle="modal" data-bs-target="#updateBirthDateModal">
                            Set Birth Date
                        </button>
                    @endif
                ...
                <div class="modal-body">
                    <form action="{{ route('student.profile.update') }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="phone_number" class="form-control" value="{{ old('phone_number', optional($user->biodata)->phone_number ?? '') }}" placeholder="Enter your phone number" required>
                        </div>

                        <!-- ID Number -->
                        <div class="mb-3">
                            <label for="id_number" class="form-label">ID Number</label>
                            <input type="text" name="id_number" class="form-control" value="{{ old('id_number', optional($user->biodata)->id_number ?? '') }}" placeholder="You can use your member ID (not your ID card number)" required>
                            <small class="text-muted">ID member can be replaced with your personal ID, but it is recommended not to use your official ID card number.</small>
                        </div>
                        ...
                        <button type="submit" class="btn btn-success w-100">{{ __('Save Changes') }}</button>
                    </form>
                </div>
            </div>
        </div>
    ...
    <div class="modal fade" id="updateBirthDateModal" tabindex="-1" aria-labelledby="updateBirthDateModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('student.profile.updateBirthDate') }}" method="POST">
                        @csrf
                        ...
                        <!-- {{ __('Save Changes') }} Button -->
                        <button type="submit" class="btn btn-success w-100">Set Birth Date</button>
                    </form>
                </div>
            </div>
        </div>
    ...
    <div class="modal fade" id="changeNameModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <form action="{{ route('student.profile.updateName') }}" method="POST">
                    @csrf
                    <div class="modal-header">
